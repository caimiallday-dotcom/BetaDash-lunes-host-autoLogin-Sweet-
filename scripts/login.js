...
const LOGIN_URL = 'https://control.optiklink.com/auth/login';
...
async function main() {
  ...
  try {
    ...
    if (!stillOnLogin || successHint > 0) {
      ...
      // **进入服务器详情**
      const serverLink = page.locator('a[href="/server/1ad8d445"]');
      ...
      // **点击 Console 菜单**
      const consoleMenu = page.locator('a[href="/server/1ad8d445"].active');
